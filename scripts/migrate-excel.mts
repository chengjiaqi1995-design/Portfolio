 ...
 *
 * 运行方式: npx tsx scripts/migrate-excel.mts
 */
...
async function main() {
  ...
  if (!fs.existsSync(excelPath)) {
    ...
    try {
      ...
      if (mainSheet) {
        const allData = XLSX.utils.sheet_to_json<unknown[]>(mainSheet, {
          header: 1,
          defval: null,
        });

        // Find the stock list section header row
        let stockStartRow = -1;
        for (let i = 0; i < allData.length; i++) {
          const row = allData[i] as unknown[];
          if (row && String(row[0] || "").includes("优先级")) {
            stockStartRow = i + 1;
            break;
          }
          // Also try English header
          if (row && String(row[9] || "") === "long/short") {
            stockStartRow = i + 1;
            break;
          }
        }
  ...
  const longCount = await prisma.position.count({ where: { longShort: "long" } });
  const shortCount = await prisma.position.count({ where: { longShort: "short" } });
  const watchCount = await prisma.position.count({ where: { longShort: "/" } });

  console.log("\n========== 迁移完成 ==========");
  console.log(`分类: ${taxCount} 条`);
  console.log(`名称映射: ${mapCount} 条`);
  console.log(`持仓总计: ${posCount} 只`);
  console.log(`  - Long: ${longCount}`);
  console.log(`  - Short: ${shortCount}`);
  console.log(`  - 观察池: ${watchCount}`);
  console.log("==============================\n");
