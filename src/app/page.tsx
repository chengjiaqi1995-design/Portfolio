...
type NetDimension = "region" | "industry" | "theme";
...
export default function DashboardPage() {
  ...
  const [netDim, setNetDim] = useState<NetDimension>("region");
  ...
  const toChartData = (data: SummaryByDimension[]) =>
    data.map((d) => ({
      name: d.name,
      long: +(d.long * 100).toFixed(1),
      short: +(d.short * 100).toFixed(1),
  ...
  const sections = [
    { title: "按地区", data: toChartData(summary.byRegion) },
    { title: "按行业", data: toChartData(summary.byIndustry) },
    { title: "按主题", data: toChartData(summary.byTheme) },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      {/* NMV Allocation — 3 charts in a row (same format as Net Exposure) */}
      <div className="grid grid-cols-3 gap-3">
        {sections.map((section) => (
          <Card key={section.title} className="py-2">
            ...
            <CardContent className="px-2 py-0">
              {section.data.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={Math.max(160, section.data.length * 28)}>
                  <BarChart
                    data={section.data}
                    layout="vertical"
                  ...
                  >
                    ...
                    />
                    <Bar dataKey="nmv" name="Net" barSize={12} radius={[0, 3, 3, 0]} isAnimationActive={false}>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="px-2 text-xs">Ticker</TableHead>
                  <TableHead className="px-2 text-xs text-right">Weight</TableHead>
                  <TableHead className="px-2 text-xs text-right">MCap(亿)</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {longPositions.map((pos, idx) => (
                  <TableRow key={pos.id} className="h-7">
                    ...
                    <TableCell className="px-2 py-1 text-xs font-mono text-right">
                      {(pos.marketCapRmb / 10).toFixed(1)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
